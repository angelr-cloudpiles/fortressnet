...
function TrafficChart({ events, series, loading, error, range }) {
  ...
  return (
    <div className="chart-wrap">
      ...
      <svg className="line-chart" viewBox="0 0 1200 300" preserveAspectRatio="xMidYMid meet">
...
function GoLiveAssistantModal({ token, platform, state, selectedTenantId, onClose, onCreated, setStatus, onNavigate }) {
  ...
  const currentStepIndex = steps.findIndex(([, complete]) => !complete);
  const completedSteps = steps.filter(([, complete]) => complete).length;
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="tenant-wizard go-live-dialog" role="dialog" aria-modal="true" aria-labelledby="go-live-title">
        <header className="tenant-wizard-header"><div><p>PROTECTED SITE</p><h2 id="go-live-title">Go-live assistant</h2></div><button className="icon-button bordered" type="button" title="Close go-live assistant" onClick={onClose}><X size={18} /></button></header>
        <div className="go-live-body">
          <aside className="go-live-progress" aria-label="Go-live progress">
            <div className="go-live-progress-heading"><span>Setup progress</span><strong>{completedSteps} of {steps.length} complete</strong></div>
            <ol className="go-live-steps">{steps.map(([label, complete, value], index) => <li key={label} className={complete ? "complete" : currentStepIndex === index ? "current" : "pending"}><span>{complete ? <CheckCircle2 size={16} /> : index + 1}</span><div><strong>{label}</strong><small>{value}</small></div></li>)}</ol>
          </aside>
          <main className="go-live-content">{!domain ? <><div className="go-live-intro"><h3>Register the first protected hostname</h3><p>Start with the public hostname and a dedicated HTTPS origin. The remaining configuration stays in its corresponding domain, DNS, origin and policy module.</p></div><DomainCreateForm token={token} tenants={state.tenants} selectedTenantId={selectedTenantId} onCreated={onCreated} setStatus={setStatus} /></> : <><OnboardingGuidance token={token} selectedTenantId={selectedTenantId} domain={domain} origin={origin} certificate={certificate} deployment={deployment} approvers={approvers} canApproveTenantChanges={tenantApprovalEligible(platform, selectedTenantId)} actorSubject={platform?.actor?.subject || ""} pendingWafChangeSet={pendingWafChangeSet} appliedWafChangeSet={appliedWafChangeSet} monitorWafChangeSet={monitorWafChangeSet} onCreated={onCreated} setStatus={setStatus} onNavigate={(target) => { onClose(); onNavigate(target); }} /><section className="go-live-instructions"><h3>DNS records and rollout status</h3><DomainInstructions domain={domain} certificate={certificate} deployment={deployment} /></section></>}</main>
        </div>
        <footer className="tenant-wizard-footer"><button className="secondary" type="button" onClick={onClose}>Finish later</button><button className="primary" type="button" onClick={() => { onClose(); onNavigate(domain ? "domains" : "overview"); }}>{domain ? "Open domain workspace" : "Back to overview"}</button></footer>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="screen">
      ...
      <Panel title="Domain workspace">
        {detailDomain ? <div className="domain-workspace">
          <label className="domain-selector" htmlFor="domain-workspace-select"><span>Selected domain</span><div><Globe2 size={17} /><select id="domain-workspace-select" value={detailDomainId} onChange={(event) => setDetailDomainId(event.target.value)}>{domains.map((domain) => <option key={domain.domain_id} value={domain.domain_id}>{domain.domain_name}</option>)}</select><ChevronDown size={16} /></div></label>
          <div className="settings-list compact"><div><Globe2 size={18} /><span>Protection status</span><strong>{humanizeWorkflowStatus(detailDomain.status)}</strong></div><div><Shield size={18} /><span>WAF baseline</span><strong>{policy ? policy.status || "Configured" : "Pending"}</strong></div><div><LockKeyhole size={18} /><span>TLS certificate</span><strong>{certificate?.status || "Pending"}</strong></div><div><Activity size={18} /><span>Edge routing</span><strong>{deployment?.status || "Not requested"}</strong></div></div>
          <div className="button-pair domain-workspace-actions"><button className="secondary compact" onClick={() => document.getElementById("domain-workspace-select")?.focus()}><Globe2 size={14} /> Change domain</button><button className="primary compact" onClick={() => onNavigate("dns")}><Shield size={14} /> Review DNS</button></div>
        </div> : <EmptyState icon={Globe2} title="No domain in this tenant" body="Add the first protected hostname to start a guided, domain-centric go-live." />}
      </Panel>
      </div>
  ...
  return (
    <div className="screen">
      ...
      <Panel className="edge-hardening-card" title="Edge Hardening">
        {deployments.length ? <div className="edge-hardening-list">{deployments.map((deployment) => <article key={deployment.deployment_id} className="edge-hardening-item"><Shield size={20} /><div className="edge-hardening-copy"><strong>{deployment.domain_name}</strong><span>Browser headers and origin TLS are managed at the edge. Origin access: {humanizeWorkflowStatus(deployment.origin_access_status || "not_checked")}.</span></div><div className="edge-hardening-actions"><button className="secondary compact" disabled={!token} onClick={() => originVerification(deployment.deployment_id, token, setStatus)}><Copy size={14} /> Origin header</button><button className="secondary compact" disabled={!token} onClick={() => edgeAction(`/api/edge-deployments/${deployment.deployment_id}/origin-access-check`, "POST", token, setStatus, onCreated, "Direct origin access check completed.")}><Activity size={14} /> Check lock</button><button className="primary compact" disabled={!token} onClick={() => edgeAction(`/api/edge-deployments/${deployment.deployment_id}/security-refresh`, "POST", token, setStatus, onCreated, "CloudFront browser security controls and origin TLS have been refreshed.")}><RefreshCw size={14} /> Refresh controls</button></div></article>)}</div> : <EmptyState icon={Shield} title="No edge deployment yet" body="Provision the protected edge before applying browser hardening and sharing the origin verification header." />}
      </Panel>
